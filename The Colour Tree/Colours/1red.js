...
addLayer("redPigment", {
    ...
    layerShown() {
        let challengeCondition = inChallenge("greenPigment", 11) || inChallenge("orangePigment", 12) || inChallenge("purplePigment", 12) || (player.stats.firstPrimary != this.layer && (inChallenge("blackPigment", 12) || inChallenge("whitePigment", 12))) || (player.stats.firstPrimary == this.layer && (inChallenge("blackPigment", 13) || inChallenge("whitePigment", 13)))
    ...
    tabFormat: [
        ...
        ["blank", "", function() {
            return (tmp.redPigment.passiveGeneration != 0 && tmp.redPigment.passiveGeneration < 1 || player.debugOptions.showAll ? {} : {display: "none"});
        }],
        ["display-text", function() {
                return "You are dying " + format(tmp.redPigment.getResetGain.mul(tmp.redPigment.passiveGeneration)) + " red pigment per second.";
        }, function() {
            return (tmp.redPigment.passiveGeneration != 0 || player.debugOptions.showAll ? {} : {display: "none"});
        }],
        "blank",
        ["upgrades", function() {
            rows = [];
            if (player.redPigment.unlocked || includesAny(player.redPigment.upgrades, [11, 12, 13]) || player.debugOptions.showAll) rows.push(1);
            if (hasUpgrade("redPigment", 13) || player.orangePigment.unlocked || includesAny(player.redPigment.upgrades, [21, 22, 23]) || player.purplePigment.unlocked || player.debugOptions.showAll) rows.push(2);
            if (hasChallenge("orangePigment", 11) || hasChallenge("purplePigment", 11) || includesAny(player.redPigment.upgrades, [31, 32, 33]) || player.debugOptions.showAll) rows.push(3);
            if (hasChallenge("orangePigment", 11) && hasChallenge("purplePigment", 11) || includesAny(player.redPigment.upgrades, [41, 42, 43]) || player.debugOptions.showAll) rows.push(4);
            if (hasChallenge("greenPigment", 12) || includesAny(player.redPigment.upgrades, [51, 52, 53]) || player.debugOptions.showAll) rows.push(5);
            return rows;
        }],
    ],
    ...
    gainMult() {
        ...
        if (layerShown("greenPigment") && hasUpgrade("greenPigment", 32)) mult = mult.mul(upgradeEffect("greenPigment", 32));
    ...
    doReset(layer) {
        ...
        if (layer == "orangePigment" || layer == "purplePigment" && hasAchievement("challenges", 22)) keepUpgrades = merge(keepUpgrades, [31, 33, 43, 53]);
        ...
        if (hasAchievement("challenges", 32)) keepUpgrades = merge(keepUpgrades, [31]);
        if (layer.indexOf("Pigment") != -1 && hasAchievement("challenges", 42)) keep = merge(keep, ["challenges", "upgrades"]);
        
        if (["orangePigment", "purplePigment", "blackPigment", "whitePigment", "greyPigment"].includes(layer)) {
    ...
    upgrades: {
        ...
        31: {
            title: "Carmine",
            description: "Red pigment acts as if it was dyed first.",
        ...
        43: {
            ...
            description: "Lose the ability to dye, but gain an additional 90% of red pigment gain per second.",
